...
string programDescription = "Updated Version of baselineSelfPairComparison that includes IMM1 baseline: This program generates random monomer sequences and tests the baseline energies from generateSelfPairBaseline AND generateBaselineIMM1, aiming to see if there is a good correlation between the calculated energy scores and the scores summed from the baseline energies. This code is test code for the baselines prior to using them for seqDesign. If there is good correlation, these energies can be used to estimate the monomer energy during design, allowing the monomer to be taken into account without separate calculation during the design process.";
