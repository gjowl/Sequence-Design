...
string programName = "baselineComparison";
string programDescription = "This program is a variant of generateRandomSequence that designs randomized sequences to evaluate baselines energies against monomer sequences: Update to evaluate only the inside 22 AAs as well due to some more varied results on the outer AAs";
string programAuthor = "Gilbert Loiseau";
string programVersion = "3";
string programDate = "2 October 2019";
...
int main(int argc, char *argv[]){
	...
	**********************************************************************************/
	...
	string noutName = dir + "/baselineEnergyComparison.out";
	string eoutName = dir + "/energies.out";
	
	pout.open(poutName.c_str());
	nout.open(noutName.c_str());
	eout.open(eoutName.c_str());
	...
	map<char,double> idEner;
	map<char,double> idEnerInner22;
	
	if (opt.weight_vdw == 1 && opt.weight_hbond == 1){
		for (uint i=0; i < opt.baselines.size(); i++){
			idEner.insert(pair<char,double>(opt.ids[i][0],opt.baselines[i]));
		}	
		
		for (uint i=0; i < opt.baselinesInner.size(); i++){
			idEnerInner22.insert(pair<char,double>(opt.ids[i][0],opt.baselinesInner[i]));
		}
	}
	else{
		for (uint i=0; i < opt.baselines.size(); i++){
			idEner.insert(pair<char,double>(opt.ids[i][0],opt.baselinesvdw[i]));
		}	
		
		for (uint i=0; i < opt.baselinesInner.size(); i++){
			idEnerInner22.insert(pair<char,double>(opt.ids[i][0],opt.baselinesvdwInner[i]));
		}
	...
	while (seqAccept < seqNumber){
		...
		//string seq = "LLLLLLLLLLLLLLLLLLLLLLLLLLLLLL";
		cout << seq << endl;
		...
		//map<char,double> idEner;
		//map<char,double> idEnerInner22;
		double monomerEner = 0;
		double monomerEnerInner22 = 0;
		
		for (uint i=0; i != seq.length(); i++){
			for (map<char,double>::iterator it = idEner.begin(); it != idEner.end(); it++) {
				if (it->first == seq[i]){
					//cout << i+1 << ": " << it->first << " " << it->second << endl;
					//nout << i+1 << ": " << it->first << " " << it->second << endl;
					monomerEner = monomerEner + it->second;
				}
			}
		}
		
		for (uint i=0; i != seq.length(); i++){
			for (map<char,double>::iterator it = idEnerInner22.begin(); it != idEnerInner22.end(); it++) {
				if (it->first == seq[i]){
					//cout << i+1 << ": " << it->first << " " << it->second << endl;
					//nout << i+1 << ": " << it->first << " " << it->second << endl;
					monomerEnerInner22 = monomerEnerInner22 + it->second;
				}
			}
		}
		...
		/******************************************************************************
		 *                  === COMPARE SEQUENCE TO BASELINE VALUES ===
		 ******************************************************************************/
		//need to import baselines somehow (I can also take the baseline energy at the beginning just by using the sequence, so maybe I should work on doing that first)
		//	if through an options vector, then import and maybe learn how to iterate through until I find the position?
		//	I think a map is easier, so I'm also going to try that out today
		if (totalEnergy < 0 && vdw < 0){
			...
			nout << "Sequence " << a+1 << ": " << seq << endl;
			nout << "Total Energy: " << totalEnergy << endl;
			nout << "Baseline Monomer Energy: " << monomerEner << endl;
			nout << "Comparison: " << totalEnergy-monomerEner << endl;
			eout << "Energy: " << totalEnergy-monomerEner << endl;
			eout << "Energy22: " << totalEnergy-monomerEnerInner22 << endl;

			cout << "Total - monomer: " << totalEnergy-monomerEner << endl;//to prove that the energies are the same since it was giving me some very small number that wasn't 0
			cout << "Total - monomer22: " << totalEnergy-monomerEnerInner22 << endl;
