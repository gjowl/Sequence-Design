...
"""
This version of the script is a refactored version of the original script. Instead of looking for a certain number of mutants,
it keeps any sequences and mutants so long as they are all below the given cutoff. numberMutants is still an options, but defaults to 0
"""
...
if __name__ == '__main__': 
    ...
    for sequence in df_wt['Sequence'].unique():
        ...
        total_clash_mutants = len(clash)
        if total_clash_mutants == 0:
            continue
        # keep the mutants that are less than the cutoff 
        if percentWtCutoff == 0 and mutantFluorCutoff != 0:
            clash = clash[clash[yAxis] < mutantFluorCutoff]
        elif mutantFluorCutoff == 0 and percentWtCutoff != 0:
            clash = clash[clash['Fluor Difference'] > percentWtCutoff]
        elif percentWtCutoff == 0 and mutantFluorCutoff == 0:
            # return all the clash mutants
            clash = clash
        else:
        ...
        if numberMutants != 0:
            ...
            if len(clash) < total_clash_mutants:
                continue
