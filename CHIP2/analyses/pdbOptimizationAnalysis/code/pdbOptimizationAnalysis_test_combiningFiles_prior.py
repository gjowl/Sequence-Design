...
if __name__ == "__main__":
    ...
    execAddPercentGpA = f'python3 {scriptDir}/addPercentGpaToDf.py -inFile {outputDir}/{dataFilename}.csv -toxgreenFile {outputDir}/{strippedSequenceFile}.csv -outFile {outputFile} -outDir {outputDir}' 
    ...
    # combine the files with the maltose data
    execCombineSequenceFiles = f'python3 {scriptDir}/combineFiles.py -seqFile {outputDir}/{sequence_maltosePassingFile}.csv -energyFile {outputDir}/{maltosePassingFile}.csv -outDir {outputDir}'
    os.system(execCombineSequenceFiles)
    execCombineMutantFiles = f'python3 {scriptDir}/combineFiles.py -seqFile {outputDir}/{mutant_maltosePassingFile}.csv -energyFile {outputDir}/{maltosePassingFile}.csv -outDir {outputDir}'
    os.system(execCombineMutantFiles)
    sequence_maltosePassingFile = f'{sequence_maltosePassingFile}_merged'
    mutant_maltosePassingFile = f'{mutant_maltosePassingFile}_merged'
            ...
            for percent_cutoff in percent_cutoffs:
                ...
                for filename in os.listdir(clashOutputDir):
                    ...
                    if os.path.isdir(file_outputDir):
                        continue
                    execAnalyzeclash = f'python3 {scriptDir}/analyzeData.py -inFile {clashOutputDir}/{filename} -outDir {file_outputDir} -percentCutoff {percent_cutoff}'
