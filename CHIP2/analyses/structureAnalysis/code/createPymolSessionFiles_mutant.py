...
def outputFrontview(input_df, output_dir, rawDataDir):
    for sequence in input_df['Sequence'].unique():
        ...
        output_file = f'{output_dir}/{sequence}.png'
...
def outputSideview(input_df, output_dir, rawDataDir):
    for sequence in input_df['Sequence'].unique():
        df_sequence = input_df[input_df['Sequence'] == sequence]
        df_sequence.reset_index(inplace=True)
        # only keep the mutant with the lowest total energy
        df_sequence = df_sequence[df_sequence['Optimized_Total'] == df_sequence['Optimized_Total'].min()]
        try:
            loadAlternatePdbs(df_sequence, pdbOptimizedDir)
        except:
            print(f'No optimized pdbs for {sequence}')
        ...
        # reset the pymol session
        cmd.reinitialize()
        # make another directory for annotated images
        annotated_dir = f'{output_dir}/annotated'
        os.makedirs(name=annotated_dir, exist_ok=True)
        ...
        # save the file with the text in the annotated directory
        output_file = f'{annotated_dir}/{sequence}.png'
