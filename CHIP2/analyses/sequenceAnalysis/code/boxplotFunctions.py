...
def plotMultiBoxplot(input_df, xaxis, yaxis, group_col, output_dir, output_filename=f"boxplot", xlabel=_sentinel, ylabel=_sentinel, ybottom=0, ytop=0, hue_order=None):
    # if ytop is not specified
    if ytop == 0:
        # get the highest value and add 1/10 of the range to the top
        ytop = input_df[yaxis].max() + input_df[yaxis].max()/10
    # check if the input dataframe is empty
    if input_df.empty:
        return
    ...
    for i, sample in enumerate(input_df[xaxis].unique()):
        ...
        for divider, group in zip(dividers, hue_order):
            ...
            if group != 'WT':
                ...
                plt.text(i + divider, height, f'p={p_value}', ha='center', fontsize=8)
