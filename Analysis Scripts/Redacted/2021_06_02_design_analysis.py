...
##############################################
#dfT = dflessPolar[dflessPolar["Total"] < -5]
df = df[df["VDWDifference"] < 0]
dfBad = df[df["Total"] > 0]

dfLAll = df[df["crossingAngle"] > 10]
dfRAll = df[df["crossingAngle"] < -10]
dfT = df[df["Total"] < 0]
...
with pd.ExcelWriter('C:\\Users\\gjowl\\Documents\\'+date+'_test.xlsx',engine='xlsxwriter') as writer:
    ...
    #dflessPolar.to_excel(writer,sheet_name='PolarAAs < 4',startrow=0 , startcol=0)
    ...
    dfLAll.to_excel(writer,sheet_name='PoorLeftHanded > 10',startrow=0 , startcol=0)
    dfRAll.to_excel(writer,sheet_name='PoorRightHanded < -10',startrow=0 , startcol=0)
