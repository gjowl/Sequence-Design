...
if __name__ == '__main__':
        ...
        if os.path.isdir(innerDir):
            ...
            for file in os.listdir(currDir):
                ...
                if file.endswith('.energy'):
                    # make sure monomer is not in the filename
                    if 'monomer' in file:
                        continue
                    ...
                    with open(filename, 'r') as f:
                        ...
                        seq = file.split('_')[0]
                        # if the sequence starts and ends with AAA, replace with LLL and ILI
                        if seq.startswith('AAA') and seq.endswith('AAA'):
                            # replace the first 3 characters with LLL
                            seq = 'LLL' + seq[3:] 
                            # replace the last 3 characters with ILI
                            seq = seq[:-3] + 'ILI'
    ...
    # keep only sequences where CHARMM_IMM1REF is greater than 0
    outputDf = outputDf[outputDf['CHARMM_IMM1REF'] > 0]
    # keep the best sequence for each sequence (lowest total energy)
    outputDf = outputDf.sort_values(by=['Total']).groupby('Sequence').last().reset_index()
