...
if __name__ == "__main__":
    ...
    sequence_col = 'InterfaceSequence'
    # remove the first and last 3 amino acids from the interface sequences
    sequence_df[sequence_col] = sequence_df[sequence_col].apply(lambda x: x[3:-3])

    # remove any - characters from the sequences
    sequence_df[sequence_col] = sequence_df[sequence_col].str.replace('-', '')

    # loop through the unique values in the design column
    for region in sequence_df['Region'].unique():
        # create a new directory for the design
        regionDir = f'{outputDir}/{region}'
        os.makedirs(regionDir, exist_ok=True)
        # create a new dataframe for the design
        design_df = sequence_df[sequence_df['Region'] == region]
        # save the design dataframe to a csv file
        design_df.to_csv(f'{regionDir}/designData.csv', index=False)

        # count the number of each amino acid in all sequences
        aaCounts = {}
        for seq in design_df[sequence_col]:
            for aa in seq:
                if aa in aaCounts:
                    aaCounts[aa] += 1
                else:
        ...
        aaCounts_df.to_csv(f'{regionDir}/aaCounts.csv')
        ...
        aaPairs_df.to_csv(f'{regionDir}/aaPairs.csv')

        # find the number of unique sequences that have each pair of amino acids
        aaPairSeqs = {}
        for pair in aaPairs:
           aaPairSeqs[pair] = len(design_df[design_df[sequence_col].str.contains(pair)])
        # convert the counts to a dataframe
        ...
        aaPairsSeqs_df.to_csv(f'{regionDir}/aaPairsSeqs.csv')